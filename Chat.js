...
fetchCustomRooms();
fetchOwnerStatus();
let activeReplyTarget = null; // { id, senderName, text }

// Function to set/open the reply bar
function setReplyTarget(id, senderName, text) {
  activeReplyTarget = { id, senderName, text };
...
function getPlatform() {
  // If running inside WebView or custom mobile agent
  return (navigator.userAgent.includes("RemixNexusApp") || window.cordova || window.Android) ? 'app' : 'web';
}

// Modify message emission to include the platform field:
// In your messageForm submit listener:
/*
socket.emit('message:send', {
  room: activeRoomId,
  text: textVal,
  replyTo: activeReplyTarget, // Attach the reply reference
  platform: getPlatform()     // 'web' or 'app' (ensuring retention rule)
});
// Reset reply after emission
activeReplyTarget = null;
document.getElementById('replyPreviewContainer').style.display = 'none';
*/

// SWIPE-TO-REPLY GESTURE RECOGNITION ON CHAT BUBBLES
function attachSwipeToReply(msgElement, messageId, senderName, messageText) {
  let startX = 0;
  let currentX = 0;
  ...
  msgElement.addEventListener('touchmove', (e) => {
    ...
    // Swipe range check (left or right swipe up to 60px max visual preview)
  ...
  msgElement.addEventListener('touchend', () => {
    let diffX = currentX - startX;
    msgElement.style.transform = 'translateX(0px)'; // Spring back to center
    
    // Swipe left or swipe right threshold of 50px
...
// Make sure to call `attachSwipeToReply(msgElement, msg.id, msg.senderName, msg.text)` 
// inside your message rendering loop in Chat.js whenever a message is drawn.