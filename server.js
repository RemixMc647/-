...
app.post("/api/block", authMiddleware, async (req,res)=>{
    ...
    if(!me.blockedUsers.includes(other.id)){

        me.blockedUsers.push(other.id);

        await me.save();

    }

    if(!other.blockedBy.includes(me.id)){

        other.blockedBy.push(me.id);
...
me.blockedUsers=
me.blockedUsers.filter(id=>id!=other.id);

other.blockedBy=
other.blockedBy.filter(id=>id!=me.id);
...
if(receiver.blockedUsers.includes(fromUserId)){
...
function isAdmin(user){
    ...
    return String(user._id) === process.env.OWNER_USER_ID;
...
async(req,res)=>{
    ...
    await RoomMessage.deleteOne({
        id:req.params.id
    });

    await DirectMessage.deleteOne({
        id:req.params.id
    });
